            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2 mt-1">
                <button
                  ...
                  onClick={() => onCopyHex(palette.primary, `Collection ${collectionNum} Primary`)}
                ...
                <button
                  ...
                  onClick={() => onCopyHex(palette.accent, `Collection ${collectionNum} Accent`)}
          ...
          {BACKGROUND_TYPE_OPTIONS.map((bg) => (
            <button
              ...
              onClick={() => {
                ...
                onCopy(`Default background set to "${bg.label}" across all project posts.`)
              }}
            >
              {bg.label}
            </button>
...
function ConfigTab({
  ...
  const slideOverridesCount = Object.keys(getStorageItem('swe-notebook-slide-overrides') || {}).length
  const collectionColorsCount = Object.keys(getStorageItem('swe-notebook-collection-colors') || {}).length
  const customAssetsCount = (getStorageItem('swe-notebook-custom-assets') || []).length

  const handleClear = (key, label) => {
    if (confirm(`Clear all ${label}?`)) {
        ...
        <div className="p-6 rounded-2xl bg-[#1a1e2a] border border-white/10 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2 p-3 bg-black/30 rounded-xl font-mono text-xs">
            <div className="flex justify-between">
              ...
              <span className="text-slate-300 font-bold">{slideOverridesCount} slides</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">Color Overrides:</span>
              <span className="text-slate-300 font-bold">{collectionColorsCount} Collections</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">Custom Assets:</span>
              <span className="text-slate-300 font-bold">{customAssetsCount} images</span>
            </div>
          </div>
          ...
          <div className="flex gap-2 flex-wrap pt-2">
            <button
              ...
              onClick={() => handleClear('swe-notebook-slide-overrides', 'slide overrides')}
            >
              🗑 Clear Slide Overrides
            </button>
            <button
              className="px-3 py-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 text-xs font-bold transition-colors cursor-pointer"
              onClick={() => handleClear('swe-notebook-collection-colors', 'color overrides')}
            >
          ...
          <button
            ...
            onClick={() => {
              const data = {
                ...
                slideOverrides: getStorageItem('swe-notebook-slide-overrides') || {},
                collectionColors: getStorageItem('swe-notebook-collection-colors') || {},
                slideAssets: getStorageItem('swe-notebook-slide-assets') || {},
              }
              const json = JSON.stringify(data, null, 2)
              onCopy('Studio Config Copied!', json)
            }}
          >
...
export default function DesignSystemPage() {
  ...
  const handleCopyConfig = (message, data) => {
    onCopy?.(data || '', message, data || '')
  }
  ...
  return (
    <div className="max-w-6xl mx-auto flex flex-col gap-8 pb-16 font-sans select-none">
      ...
      {activeTab === 'palettes' && (
        <PalettesTab collectionPalettes={collectionThemes} onCopyHex={handleCopyHex} />
      )}
      ...
      {activeTab === 'canvas' && <CanvasTab globalBg={globalBg} />}

      {activeTab === 'typography' && (
        <TypographyTab typography={designSystem.Typography || {}} />
      )}
