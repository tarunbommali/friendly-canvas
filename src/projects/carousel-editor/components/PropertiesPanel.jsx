...
// ------------------------------------------------------------------
// 2. RoundedCornerControl (image only)
// ------------------------------------------------------------------
function RoundedCornerControl({ value, onChange }) {
  const current = value ?? 0;
  const setValue = (v) => onChange(Math.max(0, v));
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-1 mt-2">
        {[
          ...
          const isSelected =
            preset.value === 99 ? current >= 80 : current === preset.value;
          return (
            <button
              key={preset.label}
              type="button"
...
// ------------------------------------------------------------------
// 3. SlidePropertiesPanel
// ------------------------------------------------------------------
function SlidePropertiesPanel() {
  const document = useCarouselStore((state) => state.document);
  const activeSlide = useCarouselStore((state) =>
    state.document.slides.find((s) => s.id === state.document.activeSlideId)
  );
  ...
  const toggleSafeAreaGuides = useCarouselStore((state) => state.toggleSafeAreaGuides);
  const updateSlideBackground = useCarouselStore((state) => state.updateSlideBackground);
  const updateSlideBgPattern = useCarouselStore((state) => state.updateSlideBgPattern);
  const updateGlobalLayoutConfig = useCarouselStore((state) => state.updateGlobalLayoutConfig);
    ...
    <div className="w-80 bg-white dark:bg-[#151821] border-l border-[#e2e8f0] dark:border-white/10 p-4 h-full flex flex-col gap-4 text-xs text-gray-700 dark:text-slate-300 overflow-y-auto shrink-0 select-none font-sans shadow-2xs">
      ...
      <div className="space-y-3">
        <div>
          ...
          <ColorInput
            ...
            onChange={(val) => updateSlideBackground(activeSlide.id, val)}
          />
        </div>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => updateSlideBgPattern(activeSlide.id, e.target.value)}
                  ...
                  onChange={(e) => {
                    ...
                    reader.onload = (evt) => {
                      ...
                      if (existingPlaceholder) {
                        useCarouselStore.getState().updateElement(existingPlaceholder.id, {
                          type: "image",
                          src: imageDataUrl,
                          isPlaceholder: false,
                        ...
                        const bounds = getLayoutBounds(
                          useCarouselStore.getState().globalLayoutConfig,
                          useCarouselStore.getState().document.metadata
                        );
                        useCarouselStore.getState().addElement({
                          id: `img_${Date.now()}`,
                          type: "image",
                          src: imageDataUrl,
                          x: bounds.canvas.width / 2,
                          y: 794,
                          width: 760,
                          height: 480,
                          originX: "center",
      ...
      <div className="border-t border-[#e2e8f0] dark:border-white/10 pt-3 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px]">
          {[
            { label: "Top", key: "safeAreaMarginTop", val: globalLayoutConfig?.safeAreaMarginTop ?? 80 },
            { label: "Bottom", key: "safeAreaMarginBottom", val: globalLayoutConfig?.safeAreaMarginBottom ?? 80 },
            { label: "Left", key: "safeAreaMarginLeft", val: globalLayoutConfig?.safeAreaMarginLeft ?? 80 },
            { label: "Right", key: "safeAreaMarginRight", val: globalLayoutConfig?.safeAreaMarginRight ?? 80 },
          ].map(({ label, key, val }) => (
            <div key={key}>
              <label className="block text-gray-500 dark:text-slate-400 mb-1">{label} (px)</label>
              <input
                type="number"
                value={val}
                onChange={(e) =>
                  updateGlobalLayoutConfig({ [key]: Number(e.target.value) })
                }
      ...
      <div className="border-t border-[#e2e8f0] dark:border-white/10 pt-3 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 text-[11px]">
          {[
            { label: "Top Clearance", key: "contentTopClearance", val: globalLayoutConfig?.contentTopClearance ?? 210 },
            { label: "Bottom Clearance", key: "contentBottomClearance", val: globalLayoutConfig?.contentBottomClearance ?? 1180 },
            { label: "Left Padding", key: "contentPaddingLeft", val: globalLayoutConfig?.contentPaddingLeft ?? 0 },
            { label: "Right Padding", key: "contentPaddingRight", val: globalLayoutConfig?.contentPaddingRight ?? 0 },
          ].map(({ label, key, val }) => (
            <div key={key}>
              <label className="block text-gray-500 dark:text-slate-400 mb-1">{label} (px)</label>
              <input
...
// 4. ElementHeader
// ------------------------------------------------------------------
function ElementHeader({ type, onCopy, onDelete, copied }) {
  return (
    <div className="flex items-center justify-between border-b border-[#e2e8f0] dark:border-white/10 pb-3">
      <div className="flex items-center gap-2 text-gray-900 dark:text-slate-100 font-bold">
        <Sliders className="w-4 h-4 text-blue-600 dark:text-blue-400" />
        <span className="capitalize">{type} Element</span>
      </div>
...
// 5. ImageProperties
// ------------------------------------------------------------------
function ImageProperties({ element, updateElement }) {
  const handleReplace = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (evt) => {
      updateElement(element.id, { src: evt.target.result });
    };
    reader.readAsDataURL(file);
  };

  const borderRadius = element.borderRadius ?? element.rx ?? 0;
  const setRadius = (val) => {
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <div className="w-full aspect-video rounded-lg overflow-hidden border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-slate-900 flex items-center justify-center p-1">
          <img
            src={element.src}
            alt="Selected element"
            style={{
              borderRadius: `${Math.min(48, borderRadius / 2)}px`,
            }}
      ...
      <RoundedCornerControl value={borderRadius} onChange={setRadius} />
    </div>
  );
}
...
function TextProperties({
  ...
  const isActiveSelection = activeTextSelection?.elementId === element.id;
  const selectedText = isActiveSelection ? activeTextSelection.selectedText : "";

  // Helper to set text alignment and position
  const setAlignment = (align, originX, x) => {
    updateElement(element.id, {
      textAlign: align,
      originX,
      x,
    ...
    <div className="space-y-3">
        ...
        <textarea
          ...
          onSelect={(e) => {
            ...
            if (typeof start === "number" && typeof end === "number" && start !== end) {
              ...
              const selectedText = (element.text || "").slice(min, max);
              setActiveTextSelection({
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <select
            value={
              element.fontFamily ||
              (element.type === "headline" || element.id?.includes("head")
                ? "Instrument Serif"
                : element.type === "badge" || element.id?.includes("badge")
                ? "Playfair Display"
                : "Georgia")
            }
            onChange={(e) =>
      ...
      <div>
        <div className="flex items-center justify-between mb-1">
          ...
          {isActiveSelection && selectedText && (
            <span
        ...
        <div className="grid grid-cols-3 gap-1.5 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-white/10 rounded p-1">
          ...
          <button
            type="button"
            onClick={() => {
              const nextStyle = element.fontStyle === "italic" ? "normal" : "italic";
          ...
          <button
            type="button"
            onClick={() => {
              const nextUnderline = !Boolean(element.underline);
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) => applyTextProperty("fontWeight", e.target.value)}
      ...
      <div>
        ...
        <div className="flex items-center gap-1 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-white/10 rounded p-1">
          <button
            type="button"
            onClick={() => setAlignment("left", "left", layoutBounds.contentZone.left)}
          ...
          <button
            type="button"
            onClick={() => setAlignment("center", "center", Math.round(layoutBounds.canvas.width / 2))}
            className={`flex-1 py-1.5 rounded flex items-center justify-center gap-1 text-xs transition-colors cursor-pointer ${
          ...
          <button
            type="button"
            onClick={() => setAlignment("right", "right", layoutBounds.contentZone.right)}
            className={`flex-1 py-1.5 rounded flex items-center justify-center gap-1 text-xs transition-colors cursor-pointer ${
      ...
      <div>
        ...
        <ColorInput
          ...
          onChange={(val) => applyTextProperty("fill", val)}
        />
      </div>
      ...
      <div>
        ...
        <ColorInput
          value={element.textBackgroundColor || "#fff176"}
          onChange={(val) => applyTextProperty("textBackgroundColor", val)}
        ...
        {element.textBackgroundColor && (
          <button
            ...
            className="mt-1 text-[10px] text-gray-600 dark:text-slate-300 hover:text-gray-900 dark:hover:text-white px-2 py-1 bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 rounded border border-gray-200 dark:border-white/10 cursor-pointer shrink-0 font-medium"
...
// ------------------------------------------------------------------
// 7. ShapeProperties (rect/circle)
// ------------------------------------------------------------------
function ShapeProperties({ element, updateElement }) {
  return (
    <div className="space-y-3">
      <div>
        <label className="block text-gray-600 dark:text-slate-400 font-semibold mb-1">
      ...
      <div>
        ...
        <ColorInput
          ...
          onChange={(val) => updateElement(element.id, { stroke: val })}
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          onChange={(e) =>
            updateElement(element.id, { strokeWidth: Number(e.target.value) })
          }
          className="w-full bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-white/10 rounded p-1.5 text-gray-800 dark:text-slate-200 font-mono focus:bg-white dark:focus:bg-slate-950 focus:border-blue-500 focus:outline-none"
        />
      </div>
...
// ------------------------------------------------------------------
// 8. TransformProperties
// ------------------------------------------------------------------
function TransformProperties({ element, updateElement, isCircle }) {
  const setDimension = (key, val) => {
    if (isCircle) {
      ...
      updateElement(element.id, { [key]: val });
    }
  };
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <input
            type="number"
            value={Math.round(element.width || (element.radius ? element.radius * 2 : 0))}
        ...
        <div>
          ...
          <input
            type="number"
            value={Math.round(element.height || (element.radius ? element.radius * 2 : 0))}
...
// ------------------------------------------------------------------
// 11. ElementPropertiesPanel (wrapper)
// ------------------------------------------------------------------
function ElementPropertiesPanel({
  ...
  const centerHorizontally = () => {
    const elWidth =
      (element.width || (element.radius ? element.radius * 2 : 400)) *
      (element.scaleX || 1);
    let newX;
    if (element.originX === "center") {
      newX = Math.round(canvasWidth / 2);
    } else if (element.originX === "right") {
  ...
  const centerVertically = () => {
    const elHeight =
      (element.height || (element.radius ? element.radius * 2 : 300)) *
      (element.scaleY || 1);
    let newY;
    if (element.originY === "center") {
      newY = Math.round(canvasHeight / 2);
    } else if (element.originY === "bottom") {
  ...
  const alignLeft = () => {
    const elWidth =
      (element.width || (element.radius ? element.radius * 2 : 0)) *
      (element.scaleX || 1);
    let newX = 0;
    if (element.originX === "center") newX = Math.round(elWidth / 2);
    else if (element.originX === "right") newX = Math.round(elWidth);
    updateElement(element.id, { x: newX });
  };
  ...
  const alignRight = () => {
    ...
    if (element.originX === "left") newX = Math.round(canvasWidth - elWidth);
    else if (element.originX === "center") {
      newX = Math.round(canvasWidth - elWidth / 2);
    }
    updateElement(element.id, { x: newX });
  };
  ...
  const distributeVertically = () => {
    const activeSlide = useCarouselStore
      .getState()
      .document.slides.find((s) => s.id === useCarouselStore.getState().document.activeSlideId);
  ...
  return (
    <div className="w-80 bg-white dark:bg-[#151821] border-l border-[#e2e8f0] dark:border-white/10 p-4 h-full flex flex-col gap-4 text-xs text-gray-700 dark:text-slate-300 overflow-y-auto shrink-0 select-none font-sans shadow-2xs">
      ...
      {/* Image specific */}
      {element.type === "image" && (
        <ImageProperties element={element} updateElement={updateElement} />
      )}
      ...
      {TEXT_TYPES.has(element.type) && (
        <TextProperties
          ...
          applyTextProperty={applyTextProperty}
          activeTextSelection={activeTextSelection}
          setActiveTextSelection={setActiveTextSelection}
          layoutBounds={layoutBounds}
        />
      )}

      {/* Shape specific (rect/circle) */}
      {(element.type === "rect" || element.type === "circle") && (
        <ShapeProperties element={element} updateElement={updateElement} />
      )}
      ...
      <AlignmentButtons
        ...
        onCenterBoth={centerBoth}
        onDistributeVertically={distributeVertically}
      />

      {/* Layer controls (only for non‑chrome elements) */}
      {!isChromeElement(element) && (
        <LayerButtons
          onNudge={(dir) => nudgeElementLayer(element.id, dir)}
        />
      )}
    </div>
  );
...
export function PropertiesPanel() {
  ...
  // applyTextProperty – used by text properties to handle selection ranges
  ...
  return (
    <ElementPropertiesPanel
      ...
      nudgeElementLayer={nudgeElementLayer}
